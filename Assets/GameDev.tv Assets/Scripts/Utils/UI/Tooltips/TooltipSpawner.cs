...
namespace GameDevTV.Core.UI.Tooltips
{
    ...
    public abstract class TooltipSpawner : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private void PositionTooltip()
        {
            ...
            Debug.Log("slotCorner = " + slotCorner);
            Debug.Log("tooltipCorner = " + tooltipCorner);
        ...
        {
            // 说明：这里原版是只有最后那一行代码，但是Unity2021版本之后会出bug，所以添加下面几行
            // bug说明：这个cs文件是挂在InventoryUISlot下面的，但是这个预制体里面又有一个InventoryItem
            // 而且它也会与鼠标互动（拖拽）会与上面发生冲突，所以在鼠标移到里面的那个对象区域时，这里的方法
            // 就直接执行了
            var slotCorners = new Vector3[4];
            GetComponent<RectTransform>().GetWorldCorners(slotCorners);
